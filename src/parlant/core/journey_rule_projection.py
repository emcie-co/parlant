...
from parlant.core.rules import Rule, RuleStore, RuleContent, RuleId
...
def format_journey_node_rule_id(
    node_id: JourneyNodeId,
    edge_id: Optional[JourneyEdgeId] = None,
    link_id: Optional[JourneyLinkId] = None,
) -> RuleId:
    if edge_id and link_id:
        return RuleId(f"journey_node:{node_id}:{edge_id}:{link_id}")

    if edge_id:
        return RuleId(f"journey_node:{node_id}:{edge_id}")

    return RuleId(f"journey_node:{node_id}")


def extract_edge_id_from_journey_node_rule_id(
    rule_id: RuleId,
) -> JourneyEdgeId | None:
    parts = rule_id.split(":")
...
def extract_node_id_from_journey_node_rule_id(
    rule_id: RuleId,
) -> JourneyNodeId:
    parts = rule_id.split(":")
    if len(parts) < 2 or parts[0] != "journey_node":
...
def extract_link_id_from_journey_node_rule_id(
    rule_id: RuleId,
) -> JourneyLinkId | None:
    parts = rule_id.split(":")
    if len(parts) < 2 or parts[0] != "journey_node":
...
class JourneyRuleProjection:
    ...
    async def project_journey_to_rules(
        self,
        ...
        def _resolve_rule_id(
            ...
            """Build rule ID using original node/edge IDs + link_id."""
            ...
            return format_journey_node_rule_id(
                original_node_id,
                original_edge_id,
                link_id,
            )
        ...
        def make_rule(
            ...
            return Rule(
                ...
                enabled=True,
                groups=list(journey.groups),
                metadata=metadata,
                composition_mode=node.composition_mode,
            )

        def add_edge_rule_metadata(rule_id: RuleId, edge_rule_id: RuleId) -> None:
            cast(dict[str, list[str]], rules[rule_id].metadata["journey_node"])["follow_ups"] = (
                list(
                    set(
                        cast(dict[str, list[str]], rules[rule_id].metadata["journey_node"])[
                            "follow_ups"
                        ]
                        + [edge_rule_id]
                    )
        ...
        while queue:
            ...
            new_rule = make_rule(edges[edge_id] if edge_id else None, nodes[node_id])
            ...
            for edge in node_edges[node_id]:
                ...
                follow_up_id = _resolve_rule_id(target_node, edge)
                add_edge_rule_metadata(new_rule.id, follow_up_id)

            visited.add((edge_id, node_id))

        # Inject evaluation results from Journey.metadata into rules.
        ...
        if node_properties:
            for rule in rules.values():
                ...
                link_id = extract_link_id_from_journey_node_rule_id(rule.id)
                ...
                for k, v in eval_props.items():
                    if k != "journey_node" and k not in rule.metadata:
                        rule.metadata[k] = v  # type: ignore[index]
