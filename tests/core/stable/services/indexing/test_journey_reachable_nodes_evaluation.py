...
def _make_rule(
    ...
    return Rule(
        ...
        content=RuleContent(condition=condition, action=action),
...
async def _evaluate_reachable(
    ...
    result = await evaluator.evaluate_reachable_follow_ups(node_rules=rules)
...
async def test_that_reachable_followups_are_correct_for_linked_sub_journey(
    ...
    rules = [root, ask_room, ask_name, validate, merge_fork, confirmed]

    reachable = await _evaluate_reachable(context, rules)
...
async def test_that_reachable_followups_work_for_chained_linked_journeys(
    ...
    rules = [root, ask_name, ask_ssn, approval]
...
async def test_that_projection_produces_valid_rules_for_reachable_evaluation(
    context: ContextOfTest,
) -> None:
    ...
    rule_store = context.container[RuleStore]
...
async def test_that_chained_linked_journeys_have_correct_node_wrapper_graph(
    ...
    node_wrappers = evaluator._build_node_wrappers(rules)
