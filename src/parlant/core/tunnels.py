...
class TunnelRequestDispatcher:
    ...
    def _get_handler(
        ...
        handlers: dict[str, Callable[[dict[str, Any]], Awaitable[dict[str, Any]]]] = {
            ...
            "groups.retrieve": self._handle_retrieve_group,
        }
    ...
    async def _handle_list_groups(self, params: dict[str, Any]) -> dict[str, Any]:
        if self._group_module is None:
            raise RuntimeError("Group module is not available")

        groups = await self._group_module.find(name=params.get("name"))
        return {"groups": [self._serialize_group(t) for t in groups]}

    async def _handle_retrieve_group(self, params: dict[str, Any]) -> dict[str, Any]:
        if self._group_module is None:
            raise RuntimeError("Group module is not available")

        try:
            group = await self._group_module.read(group_id=GroupId(params["group_id"]))
