...
async def test_that_tunnel_is_not_created_without_platform_token() -> None:
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("PARLANT_CLOUD_PLATFORM_TOKEN", None)
...
async def test_that_tunnel_is_created_with_platform_token() -> None:
    with patch.dict(os.environ, {"PARLANT_CLOUD_PLATFORM_TOKEN": "test-token"}):
        from parlant.adapters.modules.parlant_cloud import _create_tunnel_service
...
async def test_that_tunnel_uses_cloud_base_url() -> None:
    with patch.dict(
        os.environ,
        {
            "PARLANT_CLOUD_PLATFORM_TOKEN": "test-token",
