...
async def test_that_service_react_generator_disables_explicit_caching_by_default(
    logger: Logger,
) -> None:
    ...
    assert isinstance(generator, GeminiReactGenerator)
    assert not generator.cache.enabled


async def test_that_google_embedder_chunks_large_batches(logger: Logger) -> None:
    class _TestGoogleEmbedder(GoogleEmbedder):
        @property
    ...
    class _FakeModels:
        ...
        async def embed_content(self, *, model: str, contents: list[str], config: Any) -> Any:
            ...
            return SimpleNamespace(
                embeddings=[
                    SimpleNamespace(values=[float(len(self.batch_sizes)), float(index)])
                    for index, _ in enumerate(contents)
                ],
                usage_metadata=SimpleNamespace(prompt_token_count=len(contents)),
            )

    fake_models = _FakeModels()
    embedder = object.__new__(_TestGoogleEmbedder)
    embedder.logger = logger
    embedder.model_name = "gemini-embedding-001"
    embedder._client = SimpleNamespace(aio=SimpleNamespace(models=fake_models))

    result = await embedder.do_embed([f"text-{i}" for i in range(205)])

    assert fake_models.batch_sizes == [100, 100, 5]
    assert len(result.vectors) == 205
    assert result.usage.input_tokens == 205
...
def test_that_encode_forbids_tool_calls_even_with_no_tools(
    gemini: GeminiReactGenerator,
) -> None:
    # The engine disables tools by sending tools=[] AND tool_choice="none" (e.g.
    # the give-up fallback that forces a final message). Without an explicit
    # mode=NONE config the model can still emit function calls from conversation
    # history, so the disable must be passed to the API even when no tools are
    # declared.
    request = gemini._encode([], [], "none", reasoning=ReasoningConfig())
...
def test_that_encode_sends_no_tool_config_when_no_tools_and_auto(
    gemini: GeminiReactGenerator,
) -> None:
    # With no tools and the default "auto", there's nothing to constrain — leave
    # tool_config unset so the request stays minimal.
    request = gemini._encode([], [], "auto", reasoning=ReasoningConfig())
...
class _DictToolMessageStore(ToolMessageSerializer, ToolMessageDeserializer):
    ...
    def write_calls(self, calls: Any) -> None:
        self._calls = list(calls)

    def write_results(self, results: Any) -> None:
        self._results = list(results)

    def write_provider_data(self, data: Any) -> None:
        self._provider_data = dict(data)

    def read_calls(self) -> Any:
        return self._calls

    def read_results(self) -> Any:
...
def _serialize_gemini_tool_turn(
    generator: GeminiReactGenerator, *, produced_by: str
) -> _DictToolMessageStore:
    """Serialize a one-call Gemini tool turn whose call was produced by
    ``produced_by`` (the resolved per-call model) and carries a thought_signature."""
    store = _DictToolMessageStore()
    call = ToolCallPart(id="c1", name="get_weather", args={"city": "Paris"})
    call.provider_data[GEMINI_THOUGHT_SIGNATURE_KEY] = b"signature-bytes"
    call.provider_data[REACT_MODEL_KEY] = produced_by
...
def test_that_a_serialized_gemini_tool_event_records_the_resolved_model(
    gemini: GeminiReactGenerator,
) -> None:
    # The blob records the model that produced the call (resolved per-call),
    # not the generator's static identity model ("gemini-3.1-flash-lite").
    store = _serialize_gemini_tool_turn(gemini, produced_by="gemini-3.5-flash")

    assert store.read_provider_data()["model"] == "gemini-3.5-flash"


def test_that_a_gemini_tool_event_replays_natively_when_the_replay_model_matches(
    gemini: GeminiReactGenerator,
) -> None:
    store = _serialize_gemini_tool_turn(gemini, produced_by="gemini-3.5-flash")

    messages = gemini.deserialize_tool_messages(store, model="gemini-3.5-flash")

    assert messages is not None


def test_that_a_gemini_tool_event_falls_back_when_the_replay_model_differs(
    gemini: GeminiReactGenerator,
) -> None:
    store = _serialize_gemini_tool_turn(gemini, produced_by="gemini-3.5-flash")

    messages = gemini.deserialize_tool_messages(store, model="gemini-3.1-flash-lite")
...
def test_that_visible_text_starting_with_thought_prefix_raises_retryable_react_error(
    gemini: GeminiReactGenerator,
) -> None:
    with pytest.raises(ReactError) as exc:
        gemini._decode_visible_text_part("thought\nI should not be visible", TurnBuilder(), None)

    assert exc.value.retryable


def test_that_split_visible_thought_prefix_raises_before_emitting_text(
    gemini: GeminiReactGenerator,
) -> None:
    builder = TurnBuilder()

    assert gemini._decode_visible_text_part("tho", builder, None) == []

    with pytest.raises(ReactError) as exc:
        gemini._decode_visible_text_part("ught\nI should not be visible", builder, None)

    assert exc.value.retryable


def test_that_legitimate_text_sharing_thought_prefix_is_released(
    gemini: GeminiReactGenerator,
) -> None:
    builder = TurnBuilder()

    assert gemini._decode_visible_text_part("thought", builder, None) == []
    events = gemini._decode_visible_text_part("ful answer", builder, None)

    assert events == [TextDelta(text="thoughtful answer")]
    assert builder.finish().message.text == "thoughtful answer"
...
def _fake_generation_with_parts(parts: list[Any], finish_reason: str = "STOP") -> SimpleNamespace:
...
async def test_that_a_function_call_is_found_after_a_leading_thought_part(
    schematic: Gemini_3_1_Flash_Lite[_CacheProbe],
) -> None:
    # With thinking enabled, Gemini 3.x emits a thought part before the forced
    # output call. The call must be located by scanning, not assumed at index 0.
    thought = SimpleNamespace(function_call=None, thought=True, text="thinking...")
    call = SimpleNamespace(function_call=SimpleNamespace(args={"log_data": {"answer": "ranked"}}))

    async def generate_content(*args: Any, **kwargs: Any) -> SimpleNamespace:
        return _fake_generation_with_parts([thought, call])

    schematic._client = _FakeAioClient(  # type: ignore[assignment]
        schematic._client, generate=AsyncMock(side_effect=generate_content)
    )

    result = await schematic._do_generate("prompt")

    assert result.content.answer == "ranked"


async def test_that_a_missing_function_call_raises_a_retryable_error(
    schematic: Gemini_3_1_Flash_Lite[_CacheProbe],
) -> None:
    # No function call at all (e.g. the output budget was spent on thinking →
    # MAX_TOKENS). Must raise a retryable error, not crash on an assertion.
    thought = SimpleNamespace(function_call=None, thought=True, text="thinking...")

    async def generate_content(*args: Any, **kwargs: Any) -> SimpleNamespace:
        return _fake_generation_with_parts([thought], finish_reason="MAX_TOKENS")

    schematic._client = _FakeAioClient(  # type: ignore[assignment]
        schematic._client, generate=AsyncMock(side_effect=generate_content)
    )

    with pytest.raises(gemini_service.GeminiMissingFunctionCallError):
        await schematic._do_generate("prompt")
...
async def test_that_live_breakpoint_cache_reports_cached_tokens(logger: Logger) -> None:
    ...
    head = (
        "You are ranking the relevance of a rule to a conversation. "
        "Follow these standing instructions precisely. "
    ) * 120

    result = await schematic._do_generate(
        head + "\n\n# LIVE\nRule: greet the customer warmly. Answer with a relevance score.",
        hints={"cache": {"key": "live-session", "breakpoint": "# LIVE"}},
    )

    assert result.info.usage.cached_input_tokens > 0
