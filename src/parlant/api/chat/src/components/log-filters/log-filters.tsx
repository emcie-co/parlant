...
export type Type = 'RuleMatcher' | 'MessageEventComposer' | 'ToolCaller';
export type Level = 'CRITICAL' | 'ERROR' | 'WARNING' | 'INFO' | 'DEBUG' | 'TRACE';

const ALL_TYPES: Type[] = ['RuleMatcher', 'ToolCaller', 'MessageEventComposer'];
...
const typeOptions: {[key in Type]: {label: string; icon: string; color: string}} = {
	RuleMatcher: {
		label: 'Rule Matcher',
		icon: 'icons/filters/rule-matcher-color.svg',
		color: '#419480',
	},
...
// NOTE: TypeChip / CondChip / DropDownFilter are intentionally defined at module
// scope (not nested inside LogFilters). A component defined inside another
// component gets a NEW identity on every parent render, which makes React
// unmount + remount it and discard its internal state. DropDownFilter holds the
// "Edit Filters" open state (`dropdownOpen`); when LogFilters re-rendered during
// streaming status updates, the nested DropDownFilter remounted and the popover
// snapped shut the instant it opened. Keeping them at module scope gives them a
// stable identity so their state survives parent re-renders.
...
const CondChip = ({
	...
	setContentConditions?: (content: string[]) => void;
						...
						<X
							...
							onClick={(e) => {
								...
								if (apply) {
									...
									applyFn?.(sources || [], level || ALL_LEVELS[ALL_LEVELS.length - 1], newContentConditions);
								}
								deleted?.();
							}}
						/>
...
const DropDownFilter = ({def, applyFn}: {def?: {level?: Level; types?: Type[]; content?: string[]} | null; applyFn: (types: string[], level: string, content: string[]) => void}) => {
...
const LogFilters = ({
	...
	const [, setSources] = useState(structuredClone(def?.types || []));
	const [, setContentConditions] = useState(structuredClone(def?.content || []));
	const [, setLevel] = useState<Level>(def?.level || ALL_LEVELS[ALL_LEVELS.length - 1]);
		...
		<div className='flex items-center justify-between pe-[14px] z-[1] bg-white'>
			<div className={twMerge('flex z-[1] pt-[10px] pb-[8px] pe-[12px] ps-[14px] gap-[8px] h-fit min-h-[58px]', (!!def?.types?.length || !!def?.content?.length) && 'min-h-[50px]', className)}>
				<div className='filters-button flex items-start gap-[10px] flex-wrap'>
					...
					{showDropdown && <DropDownFilter def={def} applyFn={applyFn} />}
				</div>
			</div>
			{deleteFilterTab && (
