...
async def _read_journey_vector_content(store: JourneyStore, journey_id: str) -> str:
    ...
    return cast(str, vector_docs[0]["content"])
...
@pytest.mark.asyncio
async def test_that_journey_vector_queries_still_work_after_refresh_updates(
    container: Container,
) -> None:
    store = container[JourneyStore]
    _stub_embedder(store)
    ...
    node = await store.create_node(
        journey_id=journey.id,
        action="Ask for the account email",
        tools=[],
    )
    edge = await store.create_edge(
        journey_id=journey.id,
        source=journey.root_id,
        target=node.id,
        condition="customer forgot their password",
    )

    await store.update_node(node.id, {"action": "Ask for the account username"})
    await store.update_edge(edge.id, {"condition": "customer cannot sign in"})

    relevant = await store.find_relevant_journeys(
        query="customer cannot sign in",
        available_journeys=[journey],
    )

    assert [j.id for j in relevant] == [journey.id]
