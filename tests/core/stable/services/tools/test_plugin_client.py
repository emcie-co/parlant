...
from parlant.core.emission.event_buffer import EventBufferFactory
...
async def test_that_a_decorated_tool_can_be_called_directly(
    customer_context: ToolContext,
) -> None:
    @tool
    ...
    assert my_tool(customer_context, 2, None).data == 0
    assert my_tool(customer_context, 2, 1).data == 2
    assert my_tool(customer_context, 2, 2).data == 4
    assert my_tool(customer_context, arg_1=2, arg_2=3).data == 6
...
async def test_that_a_plugin_calls_a_tool(
    customer_context: ToolContext,
    container: Container,
) -> None:
    @tool
...
async def test_that_a_plugin_tool_has_access_to_the_current_session_agent_and_customer(
    ...
    async with run_service_server([my_tool]) as server:
        async with create_client(server, container[EventBufferFactory]) as client:
            ...
            assert data["session_id"] == customer_context.session_id
            assert data["agent_id"] == customer_context.agent_id
...
async def test_that_a_plugin_tool_can_emit_events(
    ...
    async with run_service_server([my_tool]) as server:
        async with create_client(
            ...
            emitted_events = buffers.for_session[SessionId(customer_context.session_id)].events
    ...
    async with run_service_server([huge_payload_tool]) as server:
        async with create_client(server, container[EventBufferFactory]) as client:
            ...
            with raises(ToolResultError) as exc:
                await client.call_tool(huge_payload_tool.tool.name, customer_context, arguments={})
