...
class RuleActionProposition(DefaultBaseModel):
    content: RuleContent
    rationale: str


class RuleActionPropositionSchema(DefaultBaseModel):
    rationale: str
    action: str
...
class RuleActionProposer:
    def __init__(
        ...
        schematic_generator: SchematicGenerator[RuleActionPropositionSchema],
    ...
    ) -> Optional[RuleActionProposition]:
        if not tool_ids or rule.action:
            return None

        if progress_report:
            await progress_report.stretch(1)

        with self._logger.scope("RuleActionProposer"):
            ...
            for generation_attempt in range(3):
                try:
                    ...
                    self._logger.warning(
                        f"RuleActionProposition attempt {generation_attempt} failed: {traceback.format_exception(exc)}"
                    )
    ...
    async def _load_tools(self, tool_ids: Sequence[ToolId]) -> list[Tool]:
        tools: list[Tool] = []

        for tid in tool_ids:
            service = await self._service_registry.read_tool_service(tid.service_name)
            try:
                tools.append(await service.read_tool(tid.tool_name))
    ...
    async def _build_prompt(
        ...
        builder.add_section(
            name="rule-action-proposer-general-instructions",
            template="""
...
Your task is given a rule's condition and a tool description (or a list of tools) to provide an action that shortly and concisely describe an action that aligns with the tool purpose.
...
""",
        ...
        builder.add_section(
            name="rule-action-proposer-example",
            template="""
Examples:
1.
...
""",
        ...
        builder.add_section(
            name="rule-action-proposer-rule",
            template="""
Rule Condition:
--------------------------
{condition}
""",
        ...
        builder.add_section(
            name="rule-action-proposer-tools",
            template="""
...
""",
    ...
    async def _generate_action(
        ...
        prompt = await self._build_prompt(rule, tools, tool_ids)
