...
"""Tests for the ReAct infrastructure (parlant.core.nlp.react) and the Gemini
adapter (parlant.adapters.nlp.gemini_service.GeminiReactGenerator).

The suite has three layers:

1. Shared-infra tests use a scripted fake provider (``_FakeReactGenerator``) to
   exercise TurnBuilder, step/stream_step/run orchestration, usage aggregation,
   history editing, and cancellation. These test the provider-agnostic core in
   ``react.py`` — they never touch (or mock) the Gemini API.
2. Gemini request-building tests check ``_encode`` and the pure
   ``_map_finish_reason`` transform offline. No API response is fabricated.
3. Live integration tests run against gemini-3.1-flash-lite, hitting the real
