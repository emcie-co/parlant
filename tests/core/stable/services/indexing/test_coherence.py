...
async def nlp_test_action_contradiction(
    ...
    customer_context = f"""Two guidelines are said to have contradicting 'then' statements if applying both of their 'then' statements would result in a contradiction or an illogical action.
...
{terms}"""
    ...
    return await nlp_test(customer_context, condition)
...
async def nlp_test_condition_entailment(
    ...
    customer_context = f"""Two guidelines should be marked as having entailing 'when' statements if the 'when' statement of one guideline entails the 'when' statement of the other, or vice-versa.
...
def base_test_that_contradicting_actions_with_hierarchical_conditions_are_detected(
    customer_context: _CustomerTestContext,
    agent: Agent,
    guideline_a_definition: dict[str, str],
    guideline_b_definition: dict[str, str],
) -> None:
    coherence_checker = customer_context.container[CoherenceChecker]
    ...
    assert customer_context.sync_await(
        incoherence_nlp_test(
            agent, customer_context.container[GlossaryStore], incoherence_results[0]
        )
    )
...
def test_that_contradicting_actions_with_hierarchical_conditions_are_detected_parametrized_1(
    ...
    base_test_that_contradicting_actions_with_hierarchical_conditions_are_detected(
        customer_context, agent, guideline_a_definition, guideline_b_definition
    )
...
def test_that_contradictory_next_message_commands_are_detected_as_incoherencies(
    ...
    for incoherence in incoherence_results:
        assert customer_context.sync_await(
            incoherence_nlp_test(agent, customer_context.container[GlossaryStore], incoherence)
        )
...
def test_that_a_glossary_based_incoherency_is_detected(
    ...
    glossary_store = customer_context.container[GlossaryStore]
