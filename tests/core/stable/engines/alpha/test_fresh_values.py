...
from tests.core.common.engines.alpha.conftest import (
    new_session,
    context,
)
...
async def test_that_value_is_not_refreshed_when_freshness_rules_are_not_met(
    ...
    application_context: ContextOfTest,
    agent_id: AgentId,
    new_session: Session,
) -> None:
    ...
    await create_fetch_account_balance_tool(application_context.container)

    context_variable_store = application_context.container[ContextVariableStore]
    service_registry = application_context.container[ServiceRegistry]
