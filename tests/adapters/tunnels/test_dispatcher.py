...
async def test_that_dispatcher_routes_groups_list() -> None:
    session_module = AsyncMock()
    group_module = AsyncMock()
    group_module.find = AsyncMock(
        return_value=[
            SimpleNamespace(
                id="group-1",
                name="Group One",
    ...
    response = await dispatcher.dispatch(
        TunnelRequest(request_id="req-groups", method="groups.list", params={})
    )

    assert response.error is None
    assert isinstance(response.result, dict)
    assert response.result["groups"][0]["id"] == "group-1"
    assert response.result["groups"][0]["name"] == "Group One"
    group_module.find.assert_awaited_once_with(name=None)


async def test_that_dispatcher_routes_groups_retrieve() -> None:
    session_module = AsyncMock()
    group_module = AsyncMock()
    group_module.read = AsyncMock(
    ...
    assert response.error is None
    assert isinstance(response.result, dict)
    assert response.result["group"]["id"] == "group-1"
    group_module.read.assert_awaited_once()
