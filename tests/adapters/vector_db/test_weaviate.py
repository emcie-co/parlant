...
from parlant.adapters.vector_db.weaviate import WeaviateCollection, WeaviateDatabase
...
@fixture
async def weaviate_database(context: _TestContext) -> AsyncIterator[WeaviateDatabase]:
    async with create_database(context) as weaviate_database:
        yield weaviate_database
...
@fixture
async def weaviate_collection(
    weaviate_database: WeaviateDatabase,
) -> AsyncIterator[WeaviateCollection[_TestDocument]]:
    collection = await weaviate_database.get_or_create_collection(
        "test_collection",
        _TestDocument,
    ...
    yield collection
    await weaviate_database.delete_collection("test_collection")


async def test_that_a_document_can_be_found_based_on_a_metadata_field(
    weaviate_collection: WeaviateCollection[_TestDocument],
    ...
    await weaviate_collection.insert_one(doc)

    find_by_id_result = await weaviate_collection.find({"id": {"$eq": "1"}})

    assert len(find_by_id_result) == 1

    assert find_by_id_result[0] == doc

    find_one_result = await weaviate_collection.find_one({"id": {"$eq": "1"}})

    assert find_one_result == doc

    find_by_name_result = await weaviate_collection.find({"name": {"$eq": "test name"}})

    assert len(find_by_name_result) == 1
    assert find_by_name_result[0] == doc

    find_by_not_existing_name_result = await weaviate_collection.find(
...
async def test_that_update_one_without_upsert_updates_existing_document(
    ...
    await weaviate_collection.insert_one(document)
    ...
    await weaviate_collection.update_one(
        {"name": {"$eq": "test name"}},
        updated_document,
        upsert=False,
    )

    result = await weaviate_collection.find({"name": {"$eq": "test name"}})
    assert len(result) == 0

    result = await weaviate_collection.find({"name": {"$eq": "new name"}})
    assert len(result) == 1
...
async def test_that_update_one_without_upsert_and_no_preexisting_document_with_same_id_does_not_insert(
    ...
    result = await weaviate_collection.update_one(
        {"name": {"$eq": "new name"}},
        updated_document,
        upsert=False,
    )
    ...
    assert 0 == len(await weaviate_collection.find({}))
...
async def test_delete_one(
    ...
    result = await weaviate_collection.find({"id": {"$eq": "1"}})
    assert len(result) == 1

    deleted_result = await weaviate_collection.delete_one({"id": {"$eq": "1"}})
...
async def test_find_similar_documents(
    ...
    await weaviate_collection.insert_one(apple_document)
    await weaviate_collection.insert_one(banana_document)
    await weaviate_collection.insert_one(cherry_document)
    await weaviate_collection.insert_one(
        _TestDocument(
            id=ObjectId("4"),
            version=doc_version,
            content="date",
    ...
    result = [s.document for s in await weaviate_collection.find_similar_documents({}, query, k)]
...
async def test_loading_collections(
    ...
    async with create_database(context) as second_db:
        fetched_collection: WeaviateCollection[_TestDocument] = await second_db.get_collection(
            "test_collection",
            _TestDocument,
            embedder_type=OpenAITextEmbedding3Large,
            document_loader=_identity_loader,
...
async def test_that_glossary_weaviate_store_correctly_finds_relevant_terms_from_large_query_input(
    container: Container,
    agent_id: AgentId,
) -> None:
    ...
    with tempfile.TemporaryDirectory():
        async with WeaviateDatabase(
            ...
            ) as glossary_weaviate_store:
                bazoo = await glossary_weaviate_store.create_term(
                    name="Bazoo",
                    description="a type of cow",
                )

                shazoo = await glossary_weaviate_store.create_term(
                    name="Shazoo",
                    description="a type of zebra",
                )

                kazoo = await glossary_weaviate_store.create_term(
                    name="Kazoo",
                    description="a type of horse",
                )

                terms = await glossary_weaviate_store.find_relevant_terms(
                    query=("walla " * 5000)
                    + "Kazoo"
                    + ("balla " * 5000)
                    + "Shazoo"
...
async def test_that_when_persistence_and_store_version_match_allows_store_to_open_when_migrate_is_disabled(
    ...
    async with create_database(context) as weaviate_db:
        async with GlossaryVectorStore(
            ...
            metadata = await weaviate_db.read_metadata()

            assert metadata
            assert metadata["version"] == GlossaryVectorStore.VERSION.to_string()


async def test_that_document_loader_updates_documents_in_current_weaviate_collection(
    context: _TestContext,
) -> None:
    ...
    async with create_database(context) as weaviate_database:
        new_collection = await weaviate_database.get_or_create_collection(
            "test_collection",
            _TestDocumentV2,
            embedder_type=OpenAITextEmbedding3Large,
...
async def test_that_failed_migrations_are_stored_in_failed_migrations_collection(
    ...
    async with create_database(context) as weaviate_database:
        ...
        collection_with_loader = await weaviate_database.get_or_create_collection(
            "test_collection",
            _TestDocumentV2,
            embedder_type=OpenAITextEmbedding3Large,
            document_loader=_document_loader,
        ...
        failed_migrations_collection = await weaviate_database.get_or_create_collection(
            "failed_migrations",
            BaseDocument,
            embedder_type=OpenAITextEmbedding3Large,
            document_loader=_identity_loader,
...
async def test_that_migration_error_raised_when_version_mismatch_and_migration_disabled(
    ...
    async with create_database(context) as weaviate_db:
        await weaviate_db.upsert_metadata(
...
async def test_that_documents_are_indexed_when_changing_embedder_type(
    ...
    async with create_database(context) as weaviate_db:
        async with GlossaryVectorStore(
            ...
            collection = await weaviate_db.get_collection(
                "glossary",
                BaseDocument,
...
async def test_and_operator_with_multiple_conditions(
    ...
    await weaviate_collection.insert_one(doc1)
    await weaviate_collection.insert_one(doc2)
    await weaviate_collection.insert_one(doc3)

    # Find documents where name is "Apple" AND id is "1"
    results = await weaviate_collection.find(
        {
            "$and": [
                {"name": {"$eq": "Apple"}},
                {"id": {"$eq": "1"}},
...
async def test_complex_nested_logical_operators(
    ...
    await weaviate_collection.insert_one(doc2)
    await weaviate_collection.insert_one(doc3)
    await weaviate_collection.insert_one(doc4)
...
async def test_and_or_with_find_similar_documents(
    ...
    results = await weaviate_collection.find_similar_documents(
        filters={
            "$or": [
                {"name": {"$eq": "Apple"}},
                {"name": {"$eq": "Banana"}},
