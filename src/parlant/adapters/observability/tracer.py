...
class ParlantCloudTracer(Tracer):
    ...
    async def __aenter__(self) -> Self:
        ...
        # Only export spans that belong to create_event operations.
        # Because _attributes ContextVar propagates inherited attributes
        # to child spans, ALL child spans within a create_event flow also
        # carry http.request.operation == "create_event" on the OTEL span.
    ...
    async def __aexit__(
        ...
        if self._processor:
            try:
                ...
                logger.warning(f"Error during ParlantCloudTracer shutdown: {e}")
    ...
    def add_event(
        ...
        # Pass all events through — the EngineTracer already controls
        # what attributes are emitted per event type.
