...
from parlant.core.engines.alpha import guideline_proposer, message_event_generator, tool_caller
...
from parlant.core.logging import FileLogger, Logger, LogLevel
from parlant.core.nlp.embedding import EmbedderFactory
from parlant.core.nlp.generation import SchematicGenerator
from parlant.core.nlp.service import NLPService
from parlant.core.persistence.migrations.migrator import validate_schema_versions
...
async def load_app(params: CLIParams) -> AsyncIterator[ASGIApplication]:
    ...
    async with setup_container(params.nlp_service) as container, EXIT_STACK:
        errs = validate_schema_versions(container)
        if errs:
...
def main() -> None:
    ...
    def migrate(
        ...
        LOGGER.warning("attempting migrations")
        nlp_service = get_nlp_service(
            openai,
            aws,
            azure,
        ...
        async def start_migration() -> None:
            ...
            async with setup_container(nlp_service) as container, EXIT_STACK:
                errs = validate_schema_versions(container)
                if not errs:
                    raise StartupError("nothing to migrate.")
