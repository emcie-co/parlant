...
@LIVE
async def test_that_live_openai_honors_a_nullable_tool_parameter(logger: Logger) -> None:
    """Proves the nullable translation works end-to-end: OpenAI returns an actual
    null for a nullable parameter (it would return "" if nullable were ignored)."""
    generator = _live_generator(logger)
    tool = ToolSpec(
        name="set_value",
