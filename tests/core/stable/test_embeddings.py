...
async def get_journeys_similarity(
    ...
    for j in journeys:
        chunk_similarities = await score_calculator.get_chunks_similarity(container, queries, j)
        similarities[j.title]["chunks"] = chunk_similarities
...
def get_queries_for_each_interaction_prefix(
    interaction_history: Sequence[Event],
    context_variables: Sequence[tuple[ContextVariable, ContextVariableValue]] = [],
    guidelines: Sequence[Guideline] = [],
    terms: Sequence[Term] = [],
