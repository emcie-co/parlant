...
async def test_that_projection_yields_followup_for_existing_rule(container: Container) -> None:
    journey_store = container[JourneyStore]
    rule_store = container[RuleStore]

    projection = JourneyRuleProjection(
    ...
    rules = await projection.project_journey_to_rules(journey.id)

    all_ids = {g.id for g in rules}
    ...
    for g in rules:
        ...
        for f_id in cast(list[str], followups):
            assert f_id in all_ids, (
                f"Bug: follow-up ID {f_id} listed in {g.id} but no rule was created for it"
            )


def _get_actions(rules: Sequence[Rule]) -> set[str | None]:
    return {g.content.action for g in rules}
...
async def test_that_projection_resolves_multi_step_sub_journey_with_tool(
    ...
    rules = await projection.project_journey_to_rules(parent.id)

    actions = _get_actions(rules)
    ...
    for g in rules:
        ...
        for f_id in cast(list[str], followups):
            assert f_id in all_ids, (
                f"Follow-up ID {f_id} listed in {g.id} but no rule was created for it"
            )

    # Verify the chain: room_type -> ask_name -> validate -> merge_fork -> confirmed/denied
    room_g = next(g for g in rules if g.content.action == "ask room type")
    room_followups = cast(
        list[str],
    ...
    name_g = [g for g in rules if g.content.action == "ask for name"]
    assert len(name_g) >= 1
    name_followups = cast(
        list[str],
    ...
    val_g = [g for g in rules if g.content.action == "validate"]
    assert len(val_g) >= 1
    val_followups = cast(
        list[str],
    ...
    # There could be multiple rules for the fork (one per incoming edge)
    fork_followup_actions: set[str | None] = set()
    for fg in fork_rules:
        fups = cast(
            list[str],
            cast(dict[str, JSONSerializable], fg.metadata["journey_node"])["follow_ups"],
        )
        for fup_id in fups:
            fup_g = next((g for g in rules if g.id == fup_id), None)
            if fup_g:
...
async def test_that_projection_resolves_sub_journey_link(container: Container) -> None:
    ...
    source_rules = [g for g in rules if g.content.action == "ask something"]
    assert len(source_rules) >= 1
    source_g = source_rules[0]
...
async def test_that_projection_includes_link_id_in_rule_ids_for_linked_nodes(
    container: Container,
) -> None:
    journey_store = container[JourneyStore]
    rule_store = container[RuleStore]
    ...
    # Sub-journey node rules should have link_id in their ID
    sub_rules = [g for g in rules if g.content.action == "sub action"]
    assert len(sub_rules) >= 1

    for g in sub_rules:
        extracted_link_id = extract_link_id_from_journey_node_rule_id(g.id)
        assert extracted_link_id == link.id, (
            f"Expected link_id={link.id} in rule ID {g.id}, got {extracted_link_id}"
        )

        # Original node_id should be extractable (not namespaced)
        extracted_node_id = extract_node_id_from_journey_node_rule_id(g.id)
        assert extracted_node_id == sub_node.id, (
            f"Expected node_id={sub_node.id} in rule ID {g.id}, got {extracted_node_id}"
        )

    # Parent-only node rules should NOT have link_id
    parent_rules = [g for g in rules if g.content.action == "parent action"]
    assert len(parent_rules) >= 1
    for g in parent_rules:
        extracted_link_id = extract_link_id_from_journey_node_rule_id(g.id)
        assert extracted_link_id is None, (
            f"Parent rule {g.id} should not have link_id, got {extracted_link_id}"
        )
...
async def test_that_same_sub_journey_linked_twice_produces_no_collisions(
    ...
    # Should have two separate rules for the shared sub_node (one per link)
    shared_rules = [g for g in rules if g.content.action == "shared action"]
    assert len(shared_rules) == 2, (
        f"Expected 2 rules for shared sub-node (one per link), got {len(shared_rules)}"
    )

    # They should have different link_ids
    link_ids = {extract_link_id_from_journey_node_rule_id(g.id) for g in shared_rules}
    assert link_ids == {link_a.id, link_b.id}, (
        f"Expected link IDs {{{link_a.id}, {link_b.id}}}, got {link_ids}"
    )

    # All rule IDs should be unique (no collisions)
    all_ids = [g.id for g in rules]
    assert len(all_ids) == len(set(all_ids)), "Rule IDs are not unique — collision detected"
...
async def test_that_projection_collapses_pass_through_forks_for_chained_linked_journeys(
    ...
    Pass-through merge_forks should be collapsed so the resulting rules
    ...
    for g in rules:
        ...
        for f_id in cast(list[str], followups):
            assert f_id in all_ids, (
                f"Follow-up ID {f_id} listed in {g.id} but no rule exists for it"
            )

    # The chain should be: root -> ask_for_ID -> ask_for_SSN -> approve_loan
    id_g = [g for g in rules if g.content.action == "ask for ID"]
    assert len(id_g) == 1
    id_followups = cast(
        list[str],
    ...
    ssn_g = [g for g in rules if g.content.action == "ask for SSN"]
    assert len(ssn_g) == 1
...
async def test_that_sub_journey_with_conditional_root_edges_preserves_conditions(
    ...
    sub = await journey_store.create_journey(
        title="Customer Routing", description="Route based on customer type", triggers=[]
    )
    ...
    # Parent journey
    parent = await journey_store.create_journey(
        title="Service Flow", description="parent", triggers=[]
    )
    ...
    # Both sub-journey branch conditions must be present in the projected rules
    onboard_g = next(g for g in rules if g.content.action == "onboard new customer")
    greet_g = next(g for g in rules if g.content.action == "greet existing customer")
    ...
    all_conditions = {g.content.condition for g in rules if g.content.condition}
    assert "needs routing" in all_conditions, (
        f"Link condition 'needs routing' missing from rules. Conditions: {all_conditions}"
    )
...
async def test_that_journey_without_node_properties_is_skipped_with_warning(
    ...
    # The engine's find_rules_for_context should skip this journey with a warning
    ...
    with warnings.catch_warnings(record=True) as caught_warnings:
        ...
        await entity_queries.find_rules_for_context(
            agent_id=agent.id,
            journeys=[journey],
        )
